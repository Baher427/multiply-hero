...
import { ArrowRight, Volume2, RotateCcw, BookOpen, Lightbulb, Grid3X3 } from 'lucide-react';
...
const WORLD_THEMES: Record<number, { emoji: string; name: string; gradient: string; color: string }> = {
  1: { emoji: '🏝️', name: 'جزيرة الأرقام', gradient: 'from-emerald-400 to-green-500', color: '#22c55e' },
  2: { emoji: '🌳', name: 'غابة الضرب', gradient: 'from-lime-400 to-emerald-500', color: '#84cc16' },
  3: { emoji: '🌊', name: 'محيط الثلاثة', gradient: 'from-teal-400 to-cyan-500', color: '#14b8a6' },
  4: { emoji: '⛰️', name: 'جبل الأربعة', gradient: 'from-amber-400 to-orange-500', color: '#f59e0b' },
  5: { emoji: '🍬', name: 'وادي الحلوى', gradient: 'from-pink-400 to-rose-500', color: '#ec4899' },
  6: { emoji: '🚀', name: 'فضاء الستة', gradient: 'from-violet-400 to-purple-500', color: '#8b5cf6' },
  7: { emoji: '🏰', name: 'قلعة السبعة', gradient: 'from-orange-400 to-red-400', color: '#f97316' },
  8: { emoji: '🎨', name: 'عالم الألوان', gradient: 'from-rose-400 to-pink-500', color: '#f43f5e' },
  9: { emoji: '👑', name: 'مملكة التسعة', gradient: 'from-yellow-400 to-amber-500', color: '#eab308' },
};
...
function getMasteryLabel(mastery: number): { label: string; color: string } {
  if (mastery >= 0.8) return { label: 'متقن ✅', color: 'text-green-600' };
  if (mastery >= 0.4) return { label: 'يتعلم 📖', color: 'text-yellow-600' };
  return { label: 'جديد 🌱', color: 'text-red-500' };
}
    ...
    >
      <div className="relative w-full" style={{ perspective: '1000px' }}>
        ...
        >
          {/* Front */}
          <div
            className={`rounded-xl p-3 md:p-4 bg-gradient-to-br ${theme.gradient} text-white text-center shadow-lg`}
          ...
          {/* Back */}
          <div
            className={`absolute inset-0 rounded-xl p-3 md:p-4 bg-gradient-to-br from-slate-700 to-slate-900 text-white text-center shadow-lg`}
...
export default function PracticeMode({
  ...
  // When a table is selected, show its details
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-slate-50 to-slate-100" dir="rtl">
        ...
        <div className="max-w-lg mx-auto p-4">
                  ...
                  <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
                    <CardContent className="p-4">
                      <h3 className="text-sm font-bold text-slate-700 mb-3 flex items-center gap-2">
                        ...
                        بطاقات جدول {selectedTable} — اضغط للكشف
                      </h3>
                      <div className="grid grid-cols-3 gap-2 md:gap-3">
                        {Array.from({ length: 9 }, (_, i) => {
                          const multiplier = i + 1;
                ...
                >
                  <Card className="border-0 shadow-lg bg-gradient-to-l from-amber-50 to-yellow-50">
                    <CardContent className="p-4">
                      ...
                      >
                        <div className="text-lg font-black mb-2">💡 {TABLE_TIPS[selectedTable]}</div>
                      </motion.div>

                      {/* Pattern highlights */}
                      <div className="space-y-3">
                        <h4 className="text-xs font-bold text-slate-600">أنماط مثيرة في هذا الجدول:</h4>
          ...
          >
            <Button
              onClick={() => onStartGame('multiple-choice', selectedTable, 'easy')}
    ...
    <div className="min-h-screen bg-gradient-to-b from-emerald-50 via-teal-50 to-cyan-50" dir="rtl">
      ...
      >
        <div className="max-w-lg mx-auto flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="text-xl md:text-2xl font-black">التدريب الحرّ 📚</h1>
      ...
      <div className="max-w-lg mx-auto p-4">
          ...
          <Card className="border-0 shadow-lg bg-white/90 backdrop-blur-sm">
            <CardContent className="p-4">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-bold text-slate-700">تقدمك الإجمالي 📊</span>
                <Badge variant="secondary" className="text-xs font-bold">
                  {Math.round((tableProgress.reduce((acc, t) => acc + t.masteryLevel, 0) / 9) * 100)}%
                </Badge>
              </div>
              <Progress
                value={(tableProgress.reduce((acc, t) => acc + t.masteryLevel, 0) / 9) * 100}
                className="h-3"
              />
        ...
        >
          <Button
            onClick={() => onStartGame('multiple-choice', 'mixed', 'easy')}
...
function getPatternHighlights(tableNumber: number): Array<{ title: string; desc: string }> {
  const patterns: Record<number, Array<{ title: string; desc: string }>> = {
    ...
    3: [
      { title: 'قاعدة القسمة على 3 🔺', desc: 'إذا كان مجموع أرقام الناتج يقبل القسمة على 3، فالناتج صحيح! 3×7=21 → 2+1=3 ✓' },
    ...
    5: [
      { title: 'القاعدة الذهبية 🖐️', desc: 'ناتج جدول 5 دائماً ينتهي بـ 0 أو 5! 5×4=20، 5×7=35' },
    ...
    9: [
      { title: 'سر الرقم 9 ✨', desc: 'مجموع أرقام أي ناتج = 9! 9×8=72 → 7+2=9، 9×6=54 → 5+4=9' },
      { title: 'حيلة الأصابع 🖐️', desc: 'ثنِّ إصبعك بالترتيب! 9×3: ثنِّ الإصبع 3 = 27 (2 أصابع يسار + 7 يمين)' },
    ],
  };
